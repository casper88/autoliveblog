...
**來源支援**:YouTube 與 Twitch 直播都已端到端實測通過。擷取管線建構在 yt-dlp 之上,它支援的網站原則上都能總結;各平台差異(開播檢查、時間戳跳轉、內嵌播放器)集中在 `autoliveblog/platforms.py`,新增平台只要在該表加一筆,未知網站走通用適配 —— 仍可正常總結,只是不提供跳轉連結。

**Podcast**:直接貼 RSS/Atom feed 網址,會抓最新一集下載音檔總結。訂閱 feed 後,有新集數就 Telegram 通知,一樣是按鈕才開始(不會自動燒額度)。啟動腳本以 Windows 為主,Python 核心跨平台。
