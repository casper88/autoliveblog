...
**平台支援**:擷取管線建構在 yt-dlp 之上,它支援的網站原則上都能總結。各平台差異(開播檢查、時間戳跳轉、內嵌播放器)集中在 `autoliveblog/platforms.py`:YouTube 完整支援,Twitch 與 Kick 已設定,其餘網站走通用適配 —— 仍可正常總結,只是不提供跳轉連結。新增平台只要在該表加一筆。啟動腳本以 Windows 為主,Python 核心跨平台。
